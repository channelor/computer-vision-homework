...
| 预训练 ViT 全参数微调 | 96.12% | 96.78% |
| 预训练 ViT linear probe | 96.01% | 96.56% |
| 有位置编码 | 99.34% | 99.34% |
| 无位置编码 | 97.34% | 97.45% |

结果表明，简化 ViT 在 MNIST 上可以完成端到端训练并收敛；位置编码能够提升模型对图像空间结构的建模能力；预训练 ViT 的全参数微调效果明显优于 linear probe；在位置编码消融实验中，有位置编码的模型性能显著优于无位置编码的模型。
