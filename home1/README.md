...
| 预训练 ResNet-18 微调 | 92.88% | 93.14% |

从零训练模型在 CIFAR-10 官方测试集上的最终准确率为 95.00%，超过 85% 的目标要求。

## 结果分析

从零训练模型最终准确率更高，主要原因是网络结构针对 CIFAR-10 的 32x32 小图像做了调整：首层卷积由原始 ResNet-18 的大卷积改为 3x3 卷积，并去掉了初始 maxpool，减少了早期下采样带来的空间信息损失。配合随机裁剪、水平翻转和余弦学习率调度，模型在 100 个 epoch 后达到 95.00% 的最终测试准确率。

预训练微调模型的优势体现在收敛速度上。它加载了 ImageNet 预训练特征，第 1 个 epoch 已达到较高测试准确率，说明通用视觉特征可以迁移到 CIFAR-10 分类任务。但 CIFAR-10 原图分辨率较低，resize 到 224x224 后不会增加真实细节；同时原始 ImageNet ResNet-18 的前几层结构并不是专门为小图像设计，因此最终准确率低于从零训练的 CIFAR-10 适配版本。
